...
function buildAssistantText(opp: Opportunity, action: string, tone: Tone, variant: number) {
  ...
  if (action === "Préparer un argumentaire") return `Angle recommandé : relier ${opp.signal.toLowerCase()} à un moment de mobilisation interne. Mettre en avant la capacité d’ALIZÉS INTERNATIONAL à concevoir un ${opp.eventType.toLowerCase()} clé en main à ${opp.destination}, avec logistique, expérience et pilotage exécutif.`;
  if (action === "Préparer un script d’appel") return `Bonjour ${contact}, je vous appelle de la part d’ALIZÉS INTERNATIONAL. Nous accompagnons les entreprises en phase de développement dans leurs événements corporate. J’ai noté plusieurs signaux autour de ${opp.company}, notamment ${opp.signal.toLowerCase()}. Est-ce que vos prochains temps forts internes ou commerciaux sont déjà planifiés ?`;
  if (action === "Générer un message LinkedIn") return `Bonjour ${contact}, j’ai vu que ${opp.company} traverse une phase active autour de ${opp.signal.toLowerCase()}. Chez ALIZÉS INTERNATIONAL, nous aidons les équipes à transformer ces moments en événements corporate premium, notamment au Maroc. Ouvert à un échange rapide ?`;
