...
export function KeywordsTypologiesPage() {
  ...
  return (
    <div className="page-fade">
      ...
      <PageHeader title="Mots-clés & typologies" subtitle="Référentiel simulé des expressions et familles d’événements utilisées par l’agent IA." action={<Guarded perm="targeting"><Button onClick={() => setOpen(true)}><Plus />Ajouter un mot-clé</Button></Guarded>} />
      <Tabs defaultValue="keywords">
        ...
        <TabsContent value="types" className="mt-5"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{eventTypes.map((type) => <Surface key={type} className="hover-lift"><div className="flex items-start justify-between gap-4"><div className="grid size-11 place-items-center rounded-xl bg-secondary text-primary"><Tag className="size-5" /></div><Switch disabled={!can("targeting")} checked={Boolean(enabledTypes[type])} onCheckedChange={() => setEnabledTypes((current) => ({ ...current, [type]: !current[type] }))} /></div><h2 className="mt-4 font-display text-lg font-semibold">{type}</h2><p className="mt-2 text-sm text-muted-foreground">Typologie surveillée pour détecter les intentions et projets événementiels.</p><div className="mt-4"><StatusBadge status={enabledTypes[type] ? "Actif" : "Inactif"} /></div></Surface>)}</div></TabsContent>
      </Tabs>
