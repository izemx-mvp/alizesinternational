      ...
      {!visible ? <SkeletonRows /> : (
        <div className={tableWrap}>
          <table className="w-full border-collapse">
            ...
            <tbody>{pageItems.map((opp) => <tr key={opp.id} className="border-b transition hover:bg-secondary/70"><td className={td}><Checkbox checked={selected.includes(opp.id)} onCheckedChange={() => toggleSelect(opp.id)} /></td><td className={td}><Link to="/opportunites/$id" params={{ id: opp.id }} className="font-semibold text-primary hover:underline">{opp.company}</Link></td><td className={td}>{opp.country}</td><td className={td}>{opp.size}</td><td className={td}><ScoreBadge score={opp.score} /></td><td className={td}>{new Date(opp.detectedAt).toLocaleDateString("fr-FR")}</td><td className={td}>{opp.opportunity}</td><td className={td}>{opp.city}</td><td className={td}>{opp.sector}</td><td className="min-w-[220px] px-4 py-3 text-sm text-muted-foreground">{opp.signal}</td><td className={td}><StatusBadge status={opp.status} /></td><td className={td}><RowActions opp={opp} onStatus={setOpportunityStatus} onNote={() => { setNoteFor(opp); setNote(""); }} onMessage={() => setMessageFor(opp)} /></td></tr>)}</tbody>
          </table>
